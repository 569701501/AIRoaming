...
export function getScriptOutlineFormatPrompt(): string {
  return [
    ...
    "只使用用户输入、项目上下文和选中的灵感种子生成内容；不要套用提示中的示例人名、剧情或设定。所有字段必须填写真实内容，不能写“待补充”。",
    "",
    "# 剧本大纲",
    "",
    "## 一、基础信息",
    ...
    "- 剧集篇幅：面向后续漫剧的总集数规划。",
    "- 剧集章数：必须是一个明确正整数，格式固定为「12 章」；不能写范围或说明文字。",
    "- 剧情简介：整部作品的一段简介。",
    "- 主要角色：列出主角、重要关系角色和主要反派，每个角色一行，格式为「角色名（定位）：描述」。",
    "- 情节概要：按集数段落规划剧情，例如「第 1 - 2 集：剧情内容」，末尾必须且只能有一行「结局方向：...」。",
    "- 章节安排：章节卡数量必须与剧集章数完全一致，章序必须从 1 连续递增；每章严格包含章节目标、核心冲突、关键转折、结尾钩子、下一章衔接五个字段。",
  ].join("\n");
}
