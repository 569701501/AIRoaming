...
- 2026-07-16 分镜及后续提示词改造顺序已确认：当前页面分镜字段和“待确认草稿 -> 用户确认 -> 正式 StoryboardVersion”流程保持不变；AI 创作与已有剧本导入在已确认 StoryVersion 后共用同一套分镜 Prompt。下一步依次为 S1 分镜生成契约/待确认草稿调整/引用映射，S2 固定质量门与一次修复，S3 双来源真实模型验收，之后再做场景参考图和候选图评测。当前已知缺口包括 `over_the_shoulder` 与正式 `over_shoulder` 不一致、AI 角色名到项目角色 ID 映射未收口。详见 `文档/04_方案与决策/2026-07-16_分镜及后续提示词改造顺序.md`。
