...
- G3-core 已于 2026-07-12 实现。G3-M0～M4 foundation 已完成；M5-A0～A3 的 capability registry、协调备份、sealed restore 与临时 restart/API happy path 已提交，M5-A4-1～A4-4 已完成并通过最终回归与双 Review，M5 状态恢复为 `completed`。D2 的 7 个 required capability、SecretStore 与 final importer 仍未闭合；D2/M6 和 production activate 保持 `prerequisite_blocked/release_blocked`，不得因 M5 完成自动推进。
