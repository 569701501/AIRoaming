...
- 2026-05-23 项目库首版已落地：包含左侧导航、项目列表、创建项目弹窗、当时的 6 步流程预览和任务队列轻量入口；项目工作区仍是下一阶段。2026-05-25 起项目库顶部搜索框已移除。2026-05-31 产品口径已更新为 7 步，需要加入项目角色库。
...
- 2026-05-27 项目 workflow 已接入：共享契约新增 `ProjectWorkflow` 和 6 步流程定义；后端按当前章节状态推导 `workflow.currentStepKey` 与 `steps` 状态，并写入 `workspace/projects/{projectId}/workflow.json`；前端流程栏按 workflow 状态显示并只开放已完成步骤和当前 active 步骤。2026-05-31 产品口径已调整为 7 步，需要新增 `project_characters` 项目角色库门槛并迁移现有 6 步实现。
- 2026-05-31 项目角色库流程收口：剧本大纲或导入剧本中的主角和常驻角色应创建为项目级 `Character` 草稿，AI 自动判断角色层级，用户可校正；主角和常驻角色必须完成角色定稿图，才能进入剧情结构；角色定稿图一旦用于生成漫画候选图，旧参考图不能覆盖，只能创建新视觉版本。
