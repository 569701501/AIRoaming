...
- G3-core 已于 2026-07-12 实现。G3-M 已从 M0 推进至 M3-A11C：maintenance、snapshot、decision/ledger 与 12 个 shadow slice 已提交；M4 只读 verifier/CLI 已有基础实现，但双 fresh shadow、API DTO/Asset 等价与复合来源摘要注册表尚未完成。Layout/Export、Dialogue/provider、full importer、DB capability、SecretStore、备份恢复和用户授权仍未满足，因此 production activate 保持 release_blocked。当前精确状态与真实 commit 以 `文档/06_测试与验收/G3-M施工包_可执行验收与Luna交接.md` 为准。
