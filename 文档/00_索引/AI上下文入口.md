...
- 2026-07-16 分镜及后续提示词改造顺序已确认，S1 已完成：当前页面分镜字段和“待确认草稿 -> 用户确认 -> 正式 StoryboardVersion”流程保持不变；AI 创作与已有剧本导入在已确认 StoryVersion 后共用同一套分镜 Prompt。生产 Prompt 已区分首次生成和调整当前 pending，修正 `over_shoulder`，完成结构本地引用到 Project Character / stable Shot ID 的映射，并让 DB 对话使用正式 ScriptVersion 与 Storyboard Working Copy。下一步是 S2 固定质量门与一次修复，再做 S3 双来源真实模型验收。详见 `文档/04_方案与决策/2026-07-16_分镜及后续提示词改造顺序.md` 和 `文档/05_执行与记录/功能完成记录/2026-07-16_分镜生成契约S1.md`。
