...
- G3-core 已于 2026-07-12 实现。G3-M0～M4 foundation 已完成；M5-A0～A3 的 capability registry、协调备份、sealed restore 与临时 restart/API happy path 已提交，M5-A4-1 一致性栅栏与 A4-2 restore release identity/ledger 已完成并复核（提交 `9d2fb2b` 与当前 A4-2 提交）。secret/路径/补偿故障矩阵和最终 rehearsal 仍未闭合，M5 当前为 `hardening_required`。下一步需另行创建 A4-3 handoff，不得直接推进 D2/M6。D2 的 7 个 required capability、SecretStore 与 final importer 仍未闭合，M6 和 production activate 保持 `prerequisite_blocked/release_blocked`。
