...
当前数据库切换事实：D2 capability=`blockedIds=[]`；S0、W1、R0B、SH-10、v5 C0～C7、首写边界和 R2 OBS-01～10 已通过，production status=`completedThrough=C7`，evidence=`sha256:987d9a9466c220544ea010b6d74ead34971b3b2eb1188388bb3a4ba66c6a1452`。G4-A～F 已总体关闭为 `G4_PASSED`；G5-M0～M4 已完成，当前进入 `G5_M5_IN_PROGRESS`。旧授权门、窗口与 `BLOCKED_R2_*` 只作历史，不能覆盖当前状态。
...
该入口采用无排期、依赖驱动执行：不设置工期、开始/结束日期或等待日期。已消费的 AUTH-C5/AUTH-C7/R2 不再重复申请；G5-M5～M8 按依赖连续执行，只在真实 blocker 或最终用户签收处暂停。G6/视频不在范围内。
...
- G1 已完成可执行 Schema/migration artifact 与 Project/Chapter/Script 最小 DB 垂直切片：显式 `AIROAMING_PERSISTENCE_MODE=db` 时，公开 Service 路径支持创建项目、保存章节草稿、完成章节，并能在应用上下文重建后从同一 SQLite 读回；连接后、业务加载前由分层 runtime catalog 精确核验正式 0001～0010。默认仍为 file，DB 模式不自动迁移，未支持写路径 fail-closed。G2 后续已接通持久任务与部分 Asset/Candidate completion，D2-A1 已完成 SecretStore；Dialogue、独立 Outbox consumer、Layout/Export、final importer 与全量 DB-only 激活仍未完成。不得把最小切片或 shadow importer 描述为 production cutover。
...
- G5 开发级方案已推进到 M4：M0 固定语料、M1/ADR-0016 技术路线、M2 strict `LayoutDocumentV1`/命令内核、M3 DB-only Working Copy/编辑器外壳，以及 M4 current source catalog、PanelFrame/FreeImage/Shot tray、模板/裁切/阅读顺序和页漫/条漫批量排版均已完成。Working Copy 自动保存与显式不可变 LayoutRevision 分离，离开编辑器不自动造版本；当前 M5 继续受控字体、富文本和气泡。正式 `layout_publication`、页漫 PNG/PDF、条漫切片/长图仍由 M7/M8 关闭。G6 已后置，不是 G0–G5 开工或签收前置。详见 `文档/04_方案与决策/2026-07-11_G5高自由成稿编辑器开发方案.md`、`文档/04_方案与决策/2026-07-11_G5LayoutDocument与编辑命令契约字典.md`、`文档/04_方案与决策/2026-07-11_G5确定性渲染与出版导出契约.md` 和 `文档/06_测试与验收/G5成稿编辑器与确定性导出验收清单.md`。
...
- 2026-07-10 D4/D5 首版边界已采纳：在条漫/页漫的有限正式成稿容器内，提供画格、图片、文字和气泡的高自由对象编排；不把成稿步骤扩张为无限白板、绘画软件或专业矢量编辑器。D45-1 至 D45-6 最终为 A/C/A/A/A/A。M2～M4 已实现 strict 文档/命令、临时多选不保存 Group、桌面/窄屏边界和完整画格/图片/模板/裁切；文字/气泡和 AI pending 仍分别由 M5/M8 完成。详见 `文档/04_方案与决策/2026-07-10_D4D5高自由成稿编辑器调研与首版边界.md` 与 `文档/04_方案与决策/ADR-0011_高自由成稿编辑器首版边界.md`。
