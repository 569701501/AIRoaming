...
- G3-core 已于 2026-07-12 实现。G3-M0～M4 foundation 已完成；M5-A0～A3 的 capability registry、协调备份、sealed restore 与临时 restart/API happy path 已提交，但 2026-07-13 独立复核发现 backup 一致性窗口、restore ledger/release identity、secret/路径/补偿故障矩阵缺口，M5 当前为 `hardening_required`。当前 Luna 入口是 `文档/05_执行与记录/任务记录/2026-07-13_G3-M5A4验收收口/handoff.md`，只领取 M5-A4-1。D2 的 7 个 required capability、SecretStore 与 final importer 仍未闭合，M6 和 production activate 保持 `prerequisite_blocked/release_blocked`。
