...
以下命令按切片逐步提供；`db:verify` 仍不得视为完成，`db:import` 目前支持 `--slice project-chapter`（A2）、`script-outline`（A3）、`script-pending-revision`（A4）、`story`（A5）和 `storyboard`（A6），不能作为 final 入口。M3-A0 额外提供一个不写 DB 的中间审计命令：
