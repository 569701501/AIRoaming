...
> **2026-07-12 实施状态：** ADR-0014 已移除自签双 Reviewer 写入门禁；44 模型 `schema.prisma` 与正式 `0001～0008` migration tree 已通过 exact check、Prisma validate 和隔离 fresh replay。显式 DB 模式已交付 Project/Chapter/Script 的创建、草稿、完章与重启读回最小切片；连接后、业务加载前会精确核验正式 migration artifact 与实际 Prisma ledger。默认仍为 file，运行时不自动 migrate，其余领域与真实 DB-only 激活仍未完成。
...
本文最初只记录开发方案；截至 2026-07-12，Schema/migration 与 Project/Chapter/Script 最小 DB 切片已经实施并留下隔离证据，但完整 G1 尚未完成。后续仍须按配套验收清单完成其余 Repository、正式 importer、持久任务、Secret/Asset/Outbox、`WIT-01` 与生产切换授权，才能把 G1 标记为完成。
