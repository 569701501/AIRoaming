...
> **2026-07-14 实施状态：** 真实 C0～C7、首笔 DB-only 写入和 R2 OBS-01～10 已完成，生产运行态为 `db_only`。正式 migration tree 已推进到 `0011_g2_project_purge_pointer_teardown`；连接后、业务加载前精确核验 migration artifact 与 Prisma ledger。file-only 在首次 DB 写后由 guard 永久拒绝。R2 的协调 purge、DB-only backup/restore、迁移 Asset storageKey 读取和 Workbench 纯读取缺口均已关闭。
...
- `PersistenceState` 与启动模式不匹配，或其 `effectiveSchemaManifestDigest` 不等于当前发布包的 effective manifest digest；`sourceManifestDigest` 只标识迁移输入 workspace/data，不能代替发布 Schema 身份。
...
正式 migration tree 与 C3 Project/Chapter/Script 最小 DB substrate 通过隔离重启读回后，G2 可以继续开发和验收，不必等待完整 `WIT-01` 或生产 M4。`WIT-01` 仍是 G1 全量 importer、七阶段语义等价与完整 DB-only witness，不因 G2 开工而降级；正式 C0～C7、真实 snapshot、真实 Secret 迁移、production `PersistenceState=db_only` 和真实用户 Runtime Review 仍是独立动作，必须在执行前重新取得用户授权。
...
- `PersistenceState=ready_for_activation`，cutoverRun/sourceManifestDigest/effectiveSchemaManifestDigest 全非空且分别与 succeeded final run 的 source 与 verification effective digest 一致，`firstBusinessWriteAt=null`。
...
- 复核 current release effective manifest identity 后，将 `PersistenceState` 原子切到 `db_only` 并记录 activatedAt；激活后 cutover run/source/effective 三项身份均不可改。
- 重新开放写入。
...
完整 V1 穷尽表不再留给实现者临场填写：Shared 10 类 task 的 codec、target/source projection、idempotency、slot/并发、attempt/backoff、lease replay、Artifact、cancel/terminal 和 stage enablement 统一见 `2026-07-11_G1任务与Outbox实施注册表.md#3-taskpolicyregistryv1`，并作为 G1 machine manifest 的必比内容。reserved 类型有完整 policy 但在所属 overlay 前固定拒绝 create。
...
- lease：60 秒。
- heartbeat：15 秒。
- `maxAttempts=3` 表示总 attempt 数，明确可重试错误只安排两次 backoff：5 秒、30 秒；第 3 次失败直接终态并清空 `nextRunAt`。`120` 不进入 runtime policy，也不存在第 4 次 attempt。
...
R2 固化的 purge 细则：删除不可变 Script/Story/Storyboard/Preflight/Layout/Export 子行前，服务显式把 Chapter reverse pointers 置空并增加 rowVersion。数据库 trigger 只在 Project=`deleting`、同项目 `project.delete_files` Outbox=`processed`、不存在 `queued/running/retrying` runtime task 三项同时成立时允许该 teardown；普通业务 pointer/source/shape/rowVersion 约束不放宽，Outbox processed 行作为审计保留。
...
1. 第一事务把 configured→clearing，仍保留 configured=1、旧 secretRef/fingerprint，并同事务插入 `secret.delete_old_ref` Outbox；Settings/Task gate 从 status=clearing 拒绝新读取。
2. Outbox worker 以 ref/fingerprint 受控删除 secret；不存在视为幂等成功，权限/owner 错误保留可恢复失败，不回显旧值。
3. 只有匹配 Outbox 已 processed 后，第二事务才允许 clearing→unconfigured，同时 configured=0 并清空 ref/fingerprint。不能在第一步提前写 unconfigured，也不能在删除失败时清 ref。

`ProviderConfig.providerId` 不可原地更新；切换 provider 身份必须新建配置与 credential、切 AppPreference，再按上述 clear 流程清旧配置。普通设置编辑只更新 displayName/modelId/baseUrl/enabled。
...
`db-only-coordinated` 备份不得伪装成 shadow：manifest 的 migration identity 表示建立备份时当前 release schema，`persistenceState.effectiveSchemaManifestDigest` 保留激活时的历史 cutover lineage；恢复分别校验两者，并忠实保留 `db_only/cutoverRunId/activatedAt/firstBusinessWriteAt`。
...
本文最初只记录开发方案；截至 2026-07-14，正式 importer、持久任务、Secret/Asset/Outbox、C0～C7、首写、backup/restore 和 R2 真实观察均已完成。目标库与 fresh restore 均由当前应用读出 1 项目、2 章节和 67/67 ready Asset，integrity=`ok`、FK=0、secret scan=0；Scrutiny 与 Runtime/User Review 均通过。旧 metadata/archive/backup 保留，未执行 down migration，也没有进入 G6/视频链路。
