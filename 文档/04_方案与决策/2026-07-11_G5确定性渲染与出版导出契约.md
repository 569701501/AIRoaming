...
> 2026-07-22 状态说明：本文的固定 RenderScene、pinned Chromium、`layout_publication` 与多 Artifact 出版链已经实现并签收。智能成稿重构只改变正式版本形成前的初稿规划与编辑保护，不改变本文“从不可变 LayoutRevision 正式渲染”的边界。

## 1. 文档状态与结论

本文定义 G5 的预检、渲染场景、出版 Profile、持久任务、不可变 ExportRevision、PNG/PDF/条漫切片和 manifest 契约。

- 本文曾以 `accepted` 作为开发基线；G5 M7～M8 现已实现并通过正式渲染与出版验收。
...
G1 base 已预建以下列，G5 通过 overlay 添加 runtime 状态机/current finalize 约束：
...
- `scopeKey=project` 或 `chapter:<chapterId>`；`(projectId, scopeKey, kind, revision)` 唯一，不依赖 nullable chapter unique。
- runtime `layout_publication.taskId/layoutRevisionId/profileDigest/preflightDigest/rendererVersion` 必填。
- taskId 唯一；GenerationTask.targetType=`export`、targetId=ExportRevision.id，与 Shared `GENERATION_TASK_TARGET_TYPES` 一致。`export_revision` 仅是 `GenerationTaskSource.sourceType`，用于表达下游任务引用某次不可变出版版本。
...
- runtime 的 task/layout/source/profile/preflight/renderer 从 INSERT 起不可改写，不等 ready 才锁来源；queued→rendering→ready/failed/cancelled、Artifact ready 校验和 Chapter current finalize 属 G5 overlay。
...
6. 同事务插入 queued ExportRevision、`target.type=export/target.id=ExportRevision.id` 的 GenerationTask、GenerationTaskSource 和 concurrency input。
...
`sourceProjection.sources` 与 GenerationTaskSource 逐项完全一致并在创建事务 seal；canonical sourceType、owner lookup 与 digest policy 以 G1 实施契约 `TaskSourceRegistryV1` 为准。
...
Export 与持久任务状态必须由同一个 claim/attempt 协议驱动，禁止各自独立推进：

- claim `layout_export` Task 的同一短事务把 queued ExportRevision 条件更新为 rendering；任一更新失败，Task claim、Attempt 和 slot 全部回滚。
- queued Task 被用户取消时，同一事务把 queued ExportRevision 置 cancelled；不能只取消一侧。
- rendering Task 的 retryable attempt 失败只结束当前 Attempt，Task 进入 retrying，但 ExportRevision 保持 rendering 并继续绑定原 task；下一 attempt 复用同一 ExportRevision，不创建第二条 revision。
- 最终 failed/cancelled 只能由当前 Attempt 的 claimToken-fenced finalize 同事务写 Task、Attempt 与 ExportRevision；失去 claim 的 worker 不能终结 Export。
- ready 只能由当前 Attempt 成功 finalize；Task succeeded、Attempt succeeded、Export ready、必要 current 指针与 slot release 在一个事务中完成。任何子更新失败全部回滚。
- terminal ExportRevision 与 terminal GenerationTask 必须一一同构；启动恢复发现单边 terminal 或 queued/running 映射不一致时进入 `needsReview`/maintenance，禁止猜测修复。

所有 Artifact ready 后，worker 在该唯一 finalize 短事务中：
...
6. 通过 G1 `TaskAttempt` finish 的唯一入口完成 GenerationTask/TaskAttempt，并由同一 statement/事务释放 slot；不允许先把 Export ready 后再单独完成 Task。
