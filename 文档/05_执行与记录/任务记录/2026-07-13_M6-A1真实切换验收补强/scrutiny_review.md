...
本复核确认 A1-1～A1-5 及后续负例补强的实现、定向测试、全量回归和文档留痕相互一致。该结论不等于真实切换通过，也不授予真实 workspace、真实数据库、真实 Keychain、真实 provider 或真实 C0～C7 的执行权限。
...
| 回归门禁 | 通过 | server 61 files / 425 tests；workspace typecheck；server/web build；Prisma/G1 checks |
...
- ACT-06 的直接证据是 DB-only resume 单测，未做进程级 kill；BK-02 的直接证据覆盖缺失 final run + shadow state；PATH-01 的直接证据覆盖 symlink/overlap。以上范围已在矩阵中明示，不能扩大解释为真实切换演练。
