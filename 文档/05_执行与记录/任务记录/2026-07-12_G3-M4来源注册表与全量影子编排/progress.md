...
- `pnpm --filter @airoaming/server test -- src/migration/project-chapter-shadow-importer.integration.spec.ts -t 'IMP-M4-(1[6-9]|2[01])' --pool=forks --poolOptions.forks.singleFork=true --reporter=dot`：6 项新增/相邻 M4 门禁通过（IMP-M4-16～21）。
- `pnpm --filter @airoaming/server test -- --run src/migration/project-chapter-shadow-importer.integration.spec.ts --pool=forks --poolOptions.forks.singleFork=true --reporter=dot`：49 项迁移集成测试通过，包含 IMP-M4-18～22 的摘要与 artifact 绑定门禁。
...
- 定向迁移集成：56/56 通过；新增报告 artifact 缺失/非法/摘要不一致、`db:verify` 真实 CLI 和 `db:import --slice full` 成功/blocked 真实 CLI 门禁。最终全量 server 回归为 47 个测试文件、301 tests 通过。
- [x] 补齐脱敏证据目录：`evidence/commands.md` 记录命令和结果，`evidence/verification.summary.json` 固化 16 slice、301 tests、56 项迁移集成和未授权范围。
...
- 当前代码复核（含 `IMP-M4-28`）复跑 server 全量 47 个测试文件/300 tests、workspace typecheck、G1 manifest/schema/migration check、Prisma validate、`git diff --check` 均通过；M4 继续保持 `in_progress` 等待正式签字。
