...
- `pnpm --filter @airoaming/server test -- src/migration/project-chapter-shadow-importer.integration.spec.ts --run --testTimeout=120000`：32 个迁移集成测试通过；包含 DB-only 断根重启、未知 entityType、摘要篡改、runtime 锚点和 storage-key pattern fail-closed。
- `IMP-M4-04`～`IMP-M4-06` 定向测试：3 项通过；覆盖注册类型摘要篡改、runtime 非 `runtime-bundle.json` 锚点和单文件实体错误 storage key。
- `pnpm --filter @airoaming/server test -- --run --testTimeout=120000`：45 个测试文件、269 个测试通过；包含 M4 API/Asset/DB-only/pending Dialogue、full fail-fast、摘要篡改、runtime 锚点和 storage-key pattern 门禁。
