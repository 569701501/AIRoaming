...
- `pnpm --filter @airoaming/server test -- --pool=forks --poolOptions.forks.singleFork=true --reporter=dot`：47 个测试文件、294 个测试通过；包含 M4 API/Asset/DB-only/pending Dialogue、full blocked/failed fail-fast、摘要篡改、runtime 锚点、storage-key pattern、Chapter sourceText fallback、replay 空来源、来源计数结构/行级门禁、full 16-slice 逐片计数校验、shadow-only run kind、importerVersion/decisionsDigest/reportDigest/verification attestation/decisions artifact 绑定门禁、统一 CLI format 门禁和 importer entityType 注册表契约。
