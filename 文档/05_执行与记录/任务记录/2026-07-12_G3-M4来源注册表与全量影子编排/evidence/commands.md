...
- 复核基线：`8a70758`；本轮新增 `IMP-M4-28/29/30` 仅增加 full CLI 成功/blocked/final fail-closed 回归测试，不改变 importer/verifier 生产逻辑。
- 后续验证提交：`bd5ca13`；补齐 `IMP-M4-30` 的 final CLI fail-closed 回归，未改变 importer/verifier 生产逻辑。
- 当前复跑：server 47 个测试文件、302 tests 全部通过；workspace typecheck、G1 manifest/schema/migration check、Prisma validate、`git diff --check` 全部通过。
- 工作区仍只保留既有 12 张截图删除；本轮没有将其纳入提交。

## M4 定向证据

- `IMP-M4-01～30` 与 full shadow / API / DB-only / pending Dialogue 链路：迁移集成文件 57/57 通过。
