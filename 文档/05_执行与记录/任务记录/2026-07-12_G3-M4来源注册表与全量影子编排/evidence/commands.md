...
| 命令 | 结果 |
| --- | --- |
| `corepack pnpm --filter @airoaming/server test -- --pool=forks --poolOptions.forks.singleFork=true --reporter=dot` | 47 个测试文件、301 tests 通过 |
...
- `IMP-M4-01～29` 与 full shadow / API / DB-only / pending Dialogue 链路：迁移集成文件 56/56 通过。
