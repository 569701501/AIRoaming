...
| db:verify / db:import 真实入口 | 通过 | `IMP-M4-26` 在临时 SQLite 上执行完整 `db:verify` CLI 成功路径并核对输出文件；`IMP-M4-27` 缺少 `--import-report` 在 Prisma 初始化前返回稳定参数错误；`IMP-M4-28` 验证 `db:import --slice full` CLI 的 16-slice 聚合报告、MigrationRun 和成功退出码 |
