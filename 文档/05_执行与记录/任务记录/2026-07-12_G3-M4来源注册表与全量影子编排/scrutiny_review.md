...
| verifier CLI 参数 | 通过 | `db-verify --format text` 在数据库初始化前返回 `MIGRATION_VERIFY_ARGS_INVALID`；`json` 是唯一输出格式 |
| 来源计数完整性 | 通过 | verifier 按 importer/entityType 精确比较 `counts.entityCounts` 与当前 run 来源行；A6/A9 特殊映射、replay 空来源和超额来源由 `IMP-M4-08/09` 覆盖 |
