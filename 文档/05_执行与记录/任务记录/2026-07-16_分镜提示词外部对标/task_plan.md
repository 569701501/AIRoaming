...
- 不把漫剧分镜降级成漫画画格的动态投影；两条轨道分别设计 Prompt，只共享正式剧情事实。
- 不把主观审美评分直接变成后端硬门。
...
7. 明确当前是 `comic / motion` 内容字段双轨、镜头骨架共用；第一版不改页面和 Schema，但为未来独立镜头序列保留边界。
...
- V2 第一版不改页面、Schema、输出字段、公开 Skill 和确认流程；内部必须拆成共享事实、漫画分镜 Prompt、漫剧分镜 Prompt 三套契约。
