...
| BAK-01 | coordinated sealed backup | full-shadow artifact 正好包含有序 16 slice，聚合/nested report 可重算且 16 条 succeeded run 共享 source/snapshot/decisions 身份；checkpoint/排他写阻断成功；DB 副本 integrity/FK/ledger 通过；全部 ready Asset bytes/hash 相同；manifest 与 SEALED 可重算 | `partial` |
| BAK-02 | fail-closed 故障注入 | 缺 slice/乱序/重复 run、artifact-ledger 摘要不一致、active writer、WAL 未收敛、ready Asset 缺失/篡改、secret sentinel、路径越界任一发生时退出非零且无可接受 SEALED bundle | `partial` |
| BAK-03 | 路径与报告稳定性 | 不同绝对临时根的同内容 bundle 不泄漏绝对路径；资产顺序、run summary 和 manifest 字段稳定；`pre-cutover` 当前被 capability/final gate 阻塞 | `partial` |
| RST-01 | verify-only | 完整验证 bundle、DB、Asset、ledger；目标路径保持不存在，bundle 字节不变 | `partial` |
| RST-02 | materialize | 仅接受两个不存在目标；DB 固定恢复到 dataRoot/db/airoaming.sqlite，Asset 按 storageKey 恢复；恢复后全摘要一致、maintenance 仍 closed | `partial` |
| RST-03 | restore 故障注入 | bundle/Asset/DB 篡改、非 sealed、目标已存在、symlink/重叠根、secret 命中均 fail-closed；只清理由本 run marker 创建的 staging | `partial` |
| RST-04 | 恢复后重启/API smoke | 用恢复根以 DB mode + maintenance closed 启动；项目/read-model/API 与备份前规范化语义一致；旧 metadata 缺失不影响读；不产生第一笔业务写 | `partial` |
...
- RST-01 已验证 sealed bundle、manifest/SEALED、DB integrity/FK、ledger 表存在、run summary 自摘要和 Asset 摘要；尚未逐项核对 16-run ledger/current release，故总状态为 partial。
...
- M5-A1～A3：实现与 happy-path 证据存在，但完整 BAK/RST 验收为 partial。
- M5-A4：必须完成相邻 A4 任务目录的 A4-CLI/BAK/RST/REG 清单，才可恢复 M5 completed。
- 无论 M5 是否完成，M6 都保持 `prerequisite_blocked`，直到 final importer、SecretStore、required capability 和用户授权另行全绿。
