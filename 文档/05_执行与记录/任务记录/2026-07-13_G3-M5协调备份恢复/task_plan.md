...
`ready_for_development`。M4 已正式通过；M5 可以在临时 data/workspace/SQLite 根开发和演练。M6、final import 和真实 production activate 继续阻塞。
...
| 切片 | 目标 | 允许开始条件 | 退出条件 |
| --- | --- | --- | --- |
| M5-A0 | capability registry + CLI | M4 completed | CAP-01/02 通过；`--check` 当前稳定阻塞 |
| M5-A1 | coordinated backup | A0 提交并复核 | BAK-01～03 通过；失败无 `SEALED` |
| M5-A2 | verify-only/materialize restore | A1 提交并复核 | RST-01～03 通过；只恢复到不存在的空根 |
