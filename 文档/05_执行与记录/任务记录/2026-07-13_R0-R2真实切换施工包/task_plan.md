...
```text
R0-A implementation_changes_requested
real_cutover_no_go
```
...
| R0-A4 | 全量门禁与双 Review | in_progress | 独立 Scrutiny=`passed`、Runtime=`passed_isolated`，独立提交 |
| R0-B | 真实 plan、release-specific shadow、SH-10 | blocked | C0 只读检查可执行 |
...
- Worker/Luna：当前只执行 R0-A；使用临时根，分阶段独立提交。
