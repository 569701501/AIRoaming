...
本切片先关闭 `update_character`：角色文本身份只写 Character 表，使用 DB identity-map refresh，禁止写 legacy `characters.json` 或 `LocalProject` whole-tree。角色 `in_use` 禁止修改，名称使用 project scope 唯一约束，rowVersion 递增。
