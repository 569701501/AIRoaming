...
- DB 模式 `update_character` 直接更新 Character，更新后只读 refresh；不调用 `ProjectStore.writeProjectFiles`。
...
- operation `update_character` 只有在测试证据落档后才标记 implemented；aggregate 仍 partial。
