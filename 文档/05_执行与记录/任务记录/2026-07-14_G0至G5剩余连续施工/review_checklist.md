...
当前事实：S0/W1/R0B/SH-10/C0～C7、首写边界、R2 OBS-01～10 和 G4-A/B/C 已完成，当前进入 `G4_D_IN_PROGRESS`。本清单不设置工期或日期；后续按依赖连续执行。
...
- [x] Candidate `selected/locked` 和旧 Server lock API 不再是 DB runtime 权威；Web 旧调用待 G4-E 改接新 API。
...
- [ ] Server 门禁拒绝 stale/unresolved 的新正式输出。
- [ ] 迟到任务只 historical。
...
- [ ] G4 清单 8 条真实用户路径通过。
- [ ] 影响弹窗、409 重新确认、排版 stale 摘要、历史导出有 screenshot/trace。
