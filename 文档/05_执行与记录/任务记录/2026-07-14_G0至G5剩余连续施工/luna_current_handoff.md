...
R2 DB-only 观察、G4-A～F 与 G5-M0～M3 已通过，从 G5-M4 开始把剩余主线按依赖顺序尽快连续推进到 G5 用户签收点：
...
currentState = G5_M4_IN_PROGRESS
```

已经完成：S0、W1、R0B、SH-10、C0～C7 激活、首笔 DB-only 写入、R2 OBS-01～10、R2 Scrutiny/Runtime Review、G4-A～F 与 G5-M0～M3；G4 状态=`G4_PASSED`，G5 M3 状态=`G5_M3_PASSED`，ADR-0016 已采纳。

尚未完成：G5-M4～M8、G5 用户签收。
...
M1 已完成：A 全硬门通过并由 ADR-0016 采纳；B 的 8192 高 resvg native abort 已留证。M2 已由 `e93d70f` 完成：strict Layout codecs、39 类命令、Unicode 17 grapheme、source/crop geometry、7 preset 与双 Review 均通过。M3 已由 `ec71594` 完成：0013 overlay、13 段 ledger、DB-only Working Copy CAS/autosave/recovery、公开 API、桌面/窄屏编辑器外壳与 file/db E2E 矩阵均通过。当前直接执行 M4：画格、图片、模板、裁切与阅读顺序。
