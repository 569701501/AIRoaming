...
| G5 | 高自由编辑器与确定性出版 | `completed` | M0～M8 技术验收与用户签收已完成 | `G0_G5_COMPLETE` |
...
状态：`completed`；提交=`fc9ea47`；Scrutiny=`passed`；Runtime=`passed`；最终用户签收=`passed`。
...
退出：手机 GET-only 预览、AI pending preview/apply/discard/expire、legacy convert/rebuild、0016 forward-only cutover、图片标准化门禁、尺寸变更预览/一次 Undo、20 段像素拼回、安全/可访问性与五条 Runtime 路径已通过。完整回归为 Shared 115/115、Server 568/568、G5 浏览器 8/8；用户已明确签收，当前状态=`G0_G5_COMPLETE`。
