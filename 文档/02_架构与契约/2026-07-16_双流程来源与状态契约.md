...
- 格式门和质量门共用一次修订总上限：格式错误只修格式，质量错误可定向重写薄弱语义；第二次仍失败则不进入待选灵感或待确认大纲。
...
3. AI 创作 P1/P2 已使用“Prompt 内部自检 + Server 高置信固定触发 + 一次定向重写”落地；不新增灵感、大纲或页面字段。更细的新鲜度、情绪力度和商业判断继续由模型内部自检与用户选择承担，不能用关键词规则冒充完整艺术评价。
4. AI 创作后续只做评测、提示词迭代和体验修正，不重新引入批量生成、自动切章或“继续下一章”。
