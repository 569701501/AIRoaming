...
- B4：批次逐章 materialize、verify；单章失败隔离，成功项形成密封 `kind=import` pending。
...
- 批次当前在目录确认请求内同步执行，并逐章持久化结果；尚未改造成后台断点续跑 worker，也没有用户可见的失败重试入口。
- 服务端不会主动截断原稿，严格契约要求来源 block 完整覆盖；超出模型上下文的超长稿尚未提供分层分析策略。
...
1. 将同步批次升级为可恢复的后台 worker，并增加失败项重试/继续入口；不能改变“目录只整体确认一次、章节仍逐章确认”的产品语义。
2. 为超长稿增加分层分析和跨块合并，同时继续满足完整覆盖、无静默截断和来源可追溯。
3. 扩充 P6 长稿中段覆盖、更多忠实度反例和断点恢复回归；现有 Prompt 正反触发、失败隔离和真实用户路径已经有自动测试。
4. AI 创作路线只做评测、提示词迭代和体验修正，不重新引入批量生成、自动切章或“继续下一章”。
