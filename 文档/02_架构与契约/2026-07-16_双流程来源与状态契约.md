...
AI 创作路线仍保留“采用草稿 → 编辑正文 → 完成本章”三层语义。AI 改写不会直接覆盖 Working Copy，而是继续形成 `kind=ai` 的 revision pending，用户采用后才进入编辑器。已有剧本路线没有采用、手动整理、AI 重新整理或丢弃动作；用户查看完整导入草稿后，只能确认该章或暂时不处理。
...
3. AI 创作 P1/P2/P3/P4/P5 已接入现有生产链：P1/P2/P3/P5 使用“Prompt 内部自检 + Server 高置信固定触发 + 一次定向重写”；P4 使用“内部层级识别 + 允许/保护契约 + 严格解析 + 高置信字段比较 + 一次定向修订”。这些能力都不新增灵感、大纲、章节或页面字段。更细的新鲜度、情绪力度、场景艺术判断和完整世界状态连续性继续由模型与用户判断，不能用关键词规则冒充完整艺术评价。
4. P5 只对运行时真实提供的上一章当前正式全文负责：第 1 章跳过；第 N 章的前章正式版本、全文和摘要来源门禁保持不变；后一张章节卡不能被当成已经发生的事实。章节编辑链尚未注入完整前章事实，不在本次 P5 完成范围内。
