...
1. A 路线：把已确认大纲、章节卡和前章正式正文接入动态 Prompt，并只在用户明确指令后创建密封 AI pending。
2. B 路线：接入 B1 原稿保存、B2 分析候选、B3 整体目录确认、B4 每章 materialize/verify 和 B5 直接确认。
3. 页面：在现有章节正文区域实现完整只读 pending 查看和 import 专用“确认章节”，不增加内容字段。
4. P6：补 Prompt 正反触发、长稿中段覆盖、忠实度、失败隔离和真实用户路径回归。
