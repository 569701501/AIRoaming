...
const TEXT_REQUIREMENT = /(?:文字|字样|写着|写有|刻着|刻有|显示(?:出|着|了)?(?:文字|字样|数字|编号|名称)|数字|编号|字母|标题|字幕|对话框|对白框|气泡|旁白框|拟声词|\btext\b|\bletters?\b|\bnumbers?\b|\bsubtitle\b|\bcaption\b|\bspeech bubbles?\b)/gi;
...
const GROUP_COUNT_HINT = /(?:[0-9]+|[\u4e00二两三四五六七八九十百]+|数十|几十|十余|数名|几名|一群|一队|成群|大批)\s*(?:人|名|个(?:人|人物|角色)|队员|守卫|士兵|村民|商队成员|群众)/;
...
export function findCandidateVisualIssues(input: CandidateVisualQualityInput): CandidateVisualIssue[] {
  ...
  if (LOCATION_TRANSITION.test(combined)) {
  ...
  if (collectiveCharacters.length > 0 && !GROUP_COUNT_HINT.test(combined)) {
  ...
  if (collectiveCharacters.length === 0 && individualCharacters.length > 0) {
    // 总人数只能由“画面”段声明；动作里的“其中一人/另一人”不是总人数。
    const explicitCounts = extractPersonCounts(visual);
    if (explicitCounts.length > 0 && !explicitCounts.includes(individualCharacters.length)) {
      issues.push(issue("VISUAL_SUBJECT_COUNT_CONFLICT", "blocking", "characters", `分镜绑定 ${individualCharacters.length} 个角色，但描述中的明确人数不一致。`));
    }
  }
...
export function extractCollectiveCountHint(value: string): string | null {
