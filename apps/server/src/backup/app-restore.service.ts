...
function verifyDatabase(databasePath: string): { bytes: number; sha256: `sha256:${string}` } {
  ...
  try {
    ...
    const ledger = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('migration_runs','persistence_states')").all() as Array<Record<string, unknown>>;
    if (integrity.length !== 1 || integrity[0]?.integrity_check !== "ok" || foreignKeys.length !== 0 || ledger.length !== 2) fail("RESTORE_VERIFICATION_FAILED");
  } catch (error) {
  ...
  return { bytes: 0, sha256: "sha256:" + "0".repeat(64) as `sha256:${string}` };
}

function verifyRunSummary(value: unknown, expected: BackupManifest["migration"]): void {
  ...
  const runIds = summary.slices.map((item) => record(item).runId);
  if (runIds.length !== new Set(runIds).size || runIds.some((item) => typeof item !== "string")) fail("RESTORE_VERIFICATION_FAILED");
}
...
export class AppRestoreService {
  async restore(input: RestoreInput): Promise<RestoreResult> {
    ...
    verifyRunSummary(runSummary, manifest.migration);
