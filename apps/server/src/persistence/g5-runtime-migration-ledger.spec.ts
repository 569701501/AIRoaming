...
import { G5_LEGACY_LAYOUT_CUTOVER_MIGRATION_NAME } from "./g5-legacy-layout-cutover-contract.js";
import {
...
describe("G5 runtime migration ledger", () => {
  it("accepts only the exact successful 16-row release", async () => {
    ...
    expect(expected.at(-1)).toMatchObject({
      migrationName: G5_LEGACY_LAYOUT_CUTOVER_MIGRATION_NAME,
    ...
    expect(() => assertG5RuntimeMigrationLedgerV1(expected, rows.slice(0, -1))).toThrow(
      `DB_PERSISTENCE_G5_MIGRATION_LEDGER_MISSING:${G5_LEGACY_LAYOUT_CUTOVER_MIGRATION_NAME}`,
    );
    ...
    expect(expected.some((entry) => entry.migrationName === G5_LAYOUT_PUBLICATION_MIGRATION_NAME)).toBe(true);
  });
});
