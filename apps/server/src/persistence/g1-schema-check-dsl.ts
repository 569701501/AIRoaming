...
const BINDINGS: readonly PhysicalCheckBindingV1[] = [
  ...
  explicit('chapters', 'ck_chapters_working_consistency', "(script_working_state = 'empty' AND script_working_text = '') OR (script_working_state = 'dirty' AND current_script_version_id IS NOT NULL AND script_working_text = '') OR (script_working_state IN ('clean', 'dirty') AND length(script_working_text) > 0)", '6.3,12.2', 'Minimum strong local CHECK: empty/nonempty working-state semantics are provable from this row, including a cleared working copy over an existing current ScriptVersion. Equality to currentScriptVersion digest remains cross-row and is owned by pointer/repository guards.'),
