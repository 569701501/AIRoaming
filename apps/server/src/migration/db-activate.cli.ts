...
const VALUE_FLAGS = ["--format", "--database-url", "--cutover-id", "--app-commit", "--plan-digest", "--run-id", "--source-manifest-digest", "--effective-manifest-digest", "--release-root", "--backup", "--maintenance-bundle", "--cutover-evidence-root", "--authorization-file", "--gate"] as const;
...
function parse(args: readonly string[]): { values: Record<string, string>; mode: DbActivateMode } {
  ...
  for (const flag of ["--release-root", "--backup", "--maintenance-bundle", "--cutover-evidence-root", "--authorization-file"] as const) if (!path.isAbsolute(values[flag]) || values[flag].includes("\0")) throw new ActivateCliError("ACTIVATE_ARGS_INVALID");
  if (!/^sha256:[0-9a-f]{64}$/.test(values["--plan-digest"])) throw new ActivateCliError("ACTIVATE_ARGS_INVALID");
  return { values, mode };
}

async function main(args = process.argv.slice(2)): Promise<number> {
  const { values, mode } = parse(args[0] === "--" ? args.slice(1) : args);
  ...
  try {
    ...
    const result = await new DbActivateService(prisma).activate({
      cutoverId: values["--cutover-id"],
      appCommit: values["--app-commit"],
      planDigest: values["--plan-digest"] as `sha256:${string}`,
      runId: values["--run-id"],
      ...
      maintenanceBundle: values["--maintenance-bundle"],
      cutoverEvidenceRoot: values["--cutover-evidence-root"],
