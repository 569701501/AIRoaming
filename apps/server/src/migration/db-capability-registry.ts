...
const operations: DbCapabilityOperation[] = [
  ...
  },
  {
    ...
    readStatus: "not_applicable",
    writeStatus: "retired",
    evidenceTestIds: ["src/projects/project-db-persistence.integration.spec.ts#P4-LEGACY-01: retires synchronous/bulk character reference routes in DB mode"],
    retirementReason: "批量旧入口不能携带逐角色 source freeze，DB 模式不再作为写入口。",
    replacement: "POST /api/projects/{projectId}/characters/{characterId}/reference per character",
  },
  {
  ...
  },
  {
    ...
    writeStatus: "retired",
    evidenceTestIds: ["src/projects/project-db-persistence.integration.spec.ts#P4-LEGACY-01: retires synchronous/bulk character reference routes in DB mode"],
    retirementReason: "同步 provider 出图绕过持久任务、claim fencing 和 staged Asset completion。",
    replacement: "POST /api/projects/{projectId}/characters/{characterId}/reference",
  },
  {
  ...
  },
  {
    ...
    writeStatus: "retired",
    evidenceTestIds: ["src/projects/project-db-persistence.integration.spec.ts#P4-LEGACY-01: retires synchronous/bulk character reference routes in DB mode"],
    retirementReason: "同步 provider 出图绕过持久任务、source freeze 和 SceneVisual completion。",
    replacement: "POST /api/projects/{projectId}/chapters/{chapterId}/scenes/{sceneId}/reference",
  },
  {
