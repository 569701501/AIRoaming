...
interface BoardPlan {
  ...
  updatedAt: Date;
  confirmedAt: Date;
  shots: Array<{ targetId: string; sourceKey: string; legacyId: string }>;
...
export class StoryboardShadowImporter {
  ...
  async import(snapshotPath: string, decisionsPath: string, options: { runId?: string; startedAt?: string } = {}): Promise<{ run: MigrationRunRecord; report: ComicFormatReport; decisions: MigrationDecisionArtifact }> {
    ...
    try {
      ...
      let boardCount = 0; let shotCount = 0; let projectionCount = 0; let warningCount = 0;
      ...
      for (const projectItem of projectItems) {
        ...
        if (targetProject) await this.ledger.withTransaction(async (tx) => { for (const plan of plans) { await this.importPlan(tx, run.id, plan); if (plan.sourceStoryVersionId) { boardCount += 1; shotCount += plan.document.shots.length; projectionCount += plan.document.shots.length; } } });
      ...
      const report = createComicFormatReport(reportProjects, { warningCount, entityCounts: { StoryboardVersion: boardCount, Shot: shotCount, StoryboardShotProjection: projectionCount } });
      const finished = await this.ledger.finishRun(run.id, { status: report.summary.unresolvedBlockerCount > 0 ? "blocked" : "succeeded", reportDigest: report.reportDigest, counts: { ...report.summary, boardCount, shotCount, projectionCount }, verification: { schemaVersion: 1, sourceManifestVerified: true, snapshotManifestVerified: true, storyboardShadowImported: true }, finishedAt: new Date().toISOString() });
  ...
  private async buildPlans(snapshot: VerifiedSnapshot, legacyProjectId: string, targetProjectId: string): Promise<BoardPlan[]> {
    ...
    for (const item of items) {
      ...
      const shotPlans = normalized.shots.sort((a, b) => a.order - b.order || a.id.localeCompare(b.id)).map((shot, index) => ({ legacyId: shot.id, sourceKey: shotSourceKey(legacyProjectId, legacyChapterId, shot.id), targetId: PrismaMigrationLedgerRepository.stableEntityId("Shot", shotSourceKey(legacyProjectId, legacyChapterId, shot.id)), order: index + 1 }));
      const shots = normalized.shots.sort((a, b) => a.order - b.order || a.id.localeCompare(b.id)).map((shot, index) => ({ ...shot, id: shotPlans[index].targetId, order: index + 1 }));
      if (shots.some((shot) => shot.characterIds.length > 0)) throw new StoryboardShadowImportError("MIGRATION_STORYBOARD_CHARACTER_UNRESOLVED");
  ...
  private async importPlan(tx: Prisma.TransactionClient, runId: string, plan: BoardPlan): Promise<void> {
    ...
    for (const shot of plan.document.shots) {
      ...
      if (!projection) await tx.storyboardShotProjection.create({ data: { id: projectionId, storyboardVersionId: plan.targetId, shotId: targetShotId, order: shot.order, storyBeatProjectionId: beat?.id ?? null, chapterSceneId: scene?.id ?? null, semanticDigest: digestCanonicalJson(shot) } });
      await this.ledger.recordImportedEntitySourceInTransaction(tx, runId, { sourceKey: projectionSource, entityType: "StoryboardShotProjection", entityId: projectionId, sourceStorageKey: plan.sourceStorageKey, sourceDigest: plan.sourceDigest, payloadDigest: digestCanonicalJson(shot), provenanceStatus: "complete" });
    }
