...
const READY_FLAGS = ["--format", "--database-url", "--run-id", "--release-root", "--workspace-root", "--secret-store-root", "--backup-verified", "--maintenance-closed"] as const;
...
function parseArgs(args: readonly string[]): ReadyArgs {
  ...
  if (values["--format"] !== "json" || (values["--backup-verified"] !== "true" && values["--backup-verified"] !== "false") || (values["--maintenance-closed"] !== "true" && values["--maintenance-closed"] !== "false")) throw new ReadyCoordinatorError("MIGRATION_READY_ARGS_INVALID");
  for (const flag of ["--release-root", "--workspace-root", "--secret-store-root"] as const) if (!path.isAbsolute(values[flag]!) || values[flag]!.includes("\0")) throw new ReadyCoordinatorError("MIGRATION_READY_ARGS_INVALID");
...
async function main(): Promise<number> {
  ...
  try {
    ...
    const result = await new ReadyCoordinator(prisma).markReady({ runId: args["--run-id"], releaseRoot: args["--release-root"], workspaceRoot: args["--workspace-root"], secretStoreRoot: args["--secret-store-root"], backupVerified: args["--backup-verified"] === "true", maintenanceClosed: args["--maintenance-closed"] === "true" });
