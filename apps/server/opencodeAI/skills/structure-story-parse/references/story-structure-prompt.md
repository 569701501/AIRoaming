...
- 「出场人物」中的尾部括号只表示出现方式或状态，不属于角色名；例如“陈姨（电话/门外）”只建立“陈姨”一张卡，括号内斜杠不能拆成两个人。整项都在括号内的新闻、遗照或舞台说明不是独立角色；正文另有明确姓名时使用该姓名。不得因电话、门外、短暂等状态重复建卡。
...
- 必须只返回符合后端固定结构约束的 JSON，不要附加说明文字。
- 不要输出评分、检查报告或新增字段。
