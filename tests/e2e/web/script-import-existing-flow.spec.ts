...
第二章 封闭总站
【本章首次整理失败】林夏用钥匙扣解锁控制面板，在封闭总站找到被困的姐姐。运营方试图销毁事故证据，姐妹二人把负责人承认掩盖事故的监控画面直播出去，最终获救。`;
...
test("B1-B5：整体确认拆章后生成全部只读待确认稿，并逐章直接形成正式版本", async ({
  ...
  await page.locator(".tool-event-trigger").last().click();
  await expect(page.getByText("整批结果：部分章节失败", { exact: true })).toBeVisible({ timeout: 30_000 });
  ...
  await page.locator(".tool-event-trigger").last().click();
  await expect(page.getByRole("button", { name: "重试本章" })).toBeVisible();
  await page.getByRole("button", { name: "重试本章" }).click();
