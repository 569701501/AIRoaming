...
test("候选图工作台可编辑单帧描述、阻断硬伤，并显式采用 AI 优化建议", async ({
  api,
  page,
  provider,
  rainSmokeProject,
  ...
  await expect(page.getByText("这里只影响本次候选图，不会改正式分镜。")).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "AI 优化描述" })).toBeEnabled();
  await page.getByRole("button", { name: "AI 优化描述" }).click();
  await expect(page.getByText("AI 优化建议")).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText("这是建议稿，尚未采用，也没有改正式分镜。")).toBeVisible();
  await action.fill("空车停稳");
  await expect(page.getByText("AI 优化建议")).not.toBeVisible();
  await action.fill("空车进站");
  await expect(page.getByText("AI 优化建议")).toBeVisible();
  await page.getByRole("button", { name: "采用优化结果" }).click();
  await expect(visual).toHaveValue(/单帧可见/);
  await expect(page.getByText("已把优化建议放入本次候选图描述；正式分镜没有改动。")).toBeVisible();
